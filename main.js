...
let enemies = ["Assasin", "Skeleton", "Zombie", "Dragon"];
let maxEnemyHealth = 75;
let enemyAttackDamage = 25;
let health = 100;
let attackDamage = 50;
let numHealthPotions = 3;
let healthPotionHealAmt = 30;
    ...
    while (enemyHealth > 0) {
        ...
        switch (control.command) {
            case "Attack":
                ...
                console.log(chalk.green(`\tYou strike the ${enemy} with ${strikeDamage} damage. `));
                console.log(chalk.red(`\tYou recieved ${damageTaken} damage from the enemy.`));
                if (health < 1) {
            ...
            case "Drink health Potion":
                if (numHealthPotions > 0) {
                    ...
                    numHealthPotions--;
                    console.log(`\tYou drank the potion, healing yourself for ${healthPotionHealAmt}.\n You now have ${health} HP\n\tYou now have ${numHealthPotions} left.`);
                }
                else {
                    console.log(`\tYou have no health potions left, defeat enemy fro a chance to get one`);
                }
                break;
            case "Run":
